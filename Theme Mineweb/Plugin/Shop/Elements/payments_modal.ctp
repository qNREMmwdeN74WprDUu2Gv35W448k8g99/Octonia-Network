  ...
  <div class="modal-dialog">
    <div class="modal-content" style="background-color: black; margin-left: auto; margin-right: auto; width: 500px;">
      ...
      <div class="modal-body">
        <?php if($isConnected AND $Permissions->can('CREDIT_ACCOUNT')) { ?>
          <?php if(!empty($starpass_offers)) { ?>
            <a class="nk-btn nk-btn-block nk-btn-lg nk-btn-color-main-1 link-effect-4 ready" data-toggle="collapse" href="#starpass" aria-expanded="false" aria-controls="starpass">StarPass</a>
            <br>
            <div class="collapse" id="starpass">
                <form method="POST" action="<?= $this->Html->url('/shop/payment/starpass') ?>">
                  ...
                  <button type="submit" class="nk-btn nk-btn-block nk-btn-lg nk-btn-color-main-1 link-effect-4 ready"><?= $Lang->get('GLOBAL__SUBMIT') ?></button>
                </form>
                <br>
            </div>
          <?php } ?>
          <?php if(!empty($dedipass)) { ?>
            <a class="nk-btn nk-btn-block nk-btn-lg nk-btn-color-main-1 link-effect-4 ready" href="<?= $this->Html->url(array('controller' => 'payment', 'action' => 'dedipass')) ?>">Dedipass</a>
          <?php } ?>
          <?php if(!empty($paypal_offers)) { ?>
            <a class="nk-btn nk-btn-block nk-btn-lg nk-btn-color-main-1 link-effect-4 ready" data-toggle="collapse" href="#PayPal" aria-expanded="false" aria-controls="PayPal">PayPal</a>
            <br>
          ...
          <?php if($paysafecard_enabled) { ?>
            <a class="nk-btn nk-btn-block nk-btn-lg nk-btn-color-main-1 link-effect-4 ready" data-toggle="collapse" href="#PaySafeCard" aria-expanded="false" aria-controls="PaySafeCard">PaySafeCard</a>
            <br>
              ...
              <form class="form-inline" data-ajax="true" action="<?= $this->Html->url(array('controller' => 'payment', 'action' => 'paysafecard')) ?>">
                ...
                <div class="form-group" style="margin-left:70px;">
                  ...
                  <div class="input-group">
                    <input type="text" class="form-control" name="amount" placeholder="€" maxlength="3" data-type="numbers" tabindex="1" style="width:60px;">
                  </div>
                    +
                </div>
                ...
                <div class="form-group">
                  ...
                  <br>
                <button type="submit" class="nk-btn nk-btn-block nk-btn-lg nk-btn-color-main-1 link-effect-4 ready pull-right"><?= $Lang->get('GLOBAL__SUBMIT') ?></button>
              </form>
            </div>
          <?php } ?>
      ...
      <div class="modal-footer">
        <button type="button" class="nk-btn nk-btn-block nk-btn-lg nk-btn-color-main-1 link-effect-4 ready" data-dismiss="modal"><?= $Lang->get('GLOBAL__CLOSE') ?></button>
        </form>
      </div>
    </div>
  </div>
