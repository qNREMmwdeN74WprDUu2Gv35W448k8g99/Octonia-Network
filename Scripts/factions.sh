...
#---------------------+
SCRNNAME="factions"
SERVICE="spigot-factions.jar"
MCPATH="/home/factions"
BACKUP_PATH="/home/factions/save"
...
MINRAM="2G"
MAXRAM="38G"
INVOCATION="java  -Xms$MINRAM -Xmx$MAXRAM -XX:PermSize=256M -XX:MaxPermSize=512M -XX:+CMSClassUnloadingEnabled -XX:+CMSPermGenSweepingEnabled -jar $SERVICE nogui"
...
mc_server_autoreboot() {
  echo " * Reboot automatique du serveur dans 5 minutes..."

  mc_alert "$STOP_WARNING_MAX"
  mc_run "broadcast Le serveur redémarre automatiquement dans 5 minutes"
  sleep 180
  mc_run "broadcast Le serveur redémarre automatiquement dans 2 minutes"
  sleep 120
  mc_run "broadcast Le serveur redémarre automatiquement dans 1 minute"
  sleep 60
  mc_run "kickall Le serveur redémarre merci de patienter, retour dans 2mn"
  mc_run "stop"
  sleep 30
  mc_stop
  sleep 5
...
case $1 in
  ...
  ;;
  *)
    echo "Usage: server {start|stop|restart|save|console|info|run|say||sreset|autoreboot|sbackup}"
  ;;
esac
exit 1 